...
class PassTurn(AuthenticatedUseCase[TurnPassOut]):
    game_id: ResourceId
    turn_id: int


@PassTurn.register_handler
class Handler(BaseHandler[PassTurn, TurnPassOut]):
    @inject
    def __init__(
        self,
    ...
    async def execute(self, user_id: ResourceId, use_case: PassTurn) -> TurnPassOut:
