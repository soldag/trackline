...
class ScoreTurn(AuthenticatedUseCase[TurnScoringOut]):
    game_id: ResourceId
    turn_id: int


@ScoreTurn.register_handler
class Handler(BaseHandler[ScoreTurn, TurnScoringOut]):
    @inject
    def __init__(
        self,
    ...
    async def execute(self, user_id: ResourceId, use_case: ScoreTurn) -> TurnScoringOut:
