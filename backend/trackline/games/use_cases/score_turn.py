...
class ScoreTurn(BaseModel):
    ...
    class Handler(BaseHandler):
        ...
        async def _score_game(
            ...
            (
                position_token_gain,
                release_year_token_gain,
                credits_token_gain,
            ), player_tokens = self._apply_token_limit(
                game,
                [position_scoring, release_year_scoring, credits_scoring],
            )

            position_scoring = position_scoring.model_copy(
                update=dict(token_gains=position_token_gain)
            )
            release_year_scoring = release_year_scoring.model_copy(
                update=dict(token_gains=release_year_token_gain)
            )
            credits_scoring = credits_scoring.model_copy(
                update=dict(token_gains=credits_token_gain),
            )
        ...
        def _apply_token_limit(
            self, game: Game, scorings: Sequence[Scoring]
        ) -> tuple[Sequence[dict[ResourceId, TokenGain]], Mapping[ResourceId, int]]:
            ...
            return scoring_token_gains, player_tokens
